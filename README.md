...
- 🎯 Entry points to execute your program using `python -m <aniplay>` or `$ aniplay` with basic CLI argument parsing.
...
[![codecov](https://codecov.io/gh/psmetildes/aniplay/branch/main/graph/badge.svg?token=aniplay_token_here)](https://codecov.io/gh/psmetildes/aniplay)
[![CI](https://github.com/psmetildes/aniplay/actions/workflows/main.yml/badge.svg)](https://github.com/psmetildes/aniplay/actions/workflows/main.yml)

Awesome aniplay created by psmetildes

## Install

from source
```bash
git clone https://github.com/psmetildes/aniplay aniplay
cd aniplay
make install
```
...
```bash
$ aniplay run --port 8080
```

or

```bash
python -m aniplay run --port 8080
```
...
```bash
❯ aniplay --help
Usage: aniplay [OPTIONS] COMMAND [ARGS]...
...
```bash
❯ aniplay create-user --help
Usage: aniplay create-user [OPTIONS] USERNAME PASSWORD
...
```bash
aniplay create-user admin admin --superuser
```
...
Run with `aniplay run` and access http://127.0.0.1:8000/docs

![](https://raw.githubusercontent.com/rochacbruno/fastapi-project-template/master/docs/api.png)


**For some api calls you must authenticate** using the user created with `aniplay create-user`.
...
-----------------------------------------------------
aniplay/__init__.py              4      0   100%
aniplay/app.py                  16      1    94%
aniplay/cli.py                  21      0   100%
aniplay/config.py                5      0   100%
aniplay/db.py                   10      0   100%
aniplay/models/__init__.py       0      0   100%
aniplay/models/content.py       47      1    98%
aniplay/routes/__init__.py      11      0   100%
aniplay/routes/content.py       52     25    52%
aniplay/routes/security.py      15      1    93%
aniplay/routes/user.py          52     26    50%
aniplay/security.py            103     12    88%
-----------------------------------------------------
TOTAL                               336     66    80%
...
```py
from aniplay.config import settings
```
...
> `dynaconf_merge` is a boolean that tells if the settings should be merged with the default settings defined in aniplay/default.toml.

### As environment variables
```bash
export aniplay_KEY=value
export aniplay_KEY="@int 42"
export aniplay_KEY="@jinja {{ this.db.uri }}"
export aniplay_DB__uri="@jinja {{ this.db.uri | replace('db', 'data') }}"
```
...
```bash
aniplay_ENV=production aniplay run
```
